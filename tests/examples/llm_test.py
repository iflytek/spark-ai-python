...
def test_gemma2b():
    from sparkai.core.callbacks import StdOutCallbackHandler
    messages = [{'role': 'user',
                 'content': '''你是一个意图识别的助手，需要从用户的问题中判断出用户的意图并以标准json的格式输出，你的意图分类包括[请假、会议、订餐、企业知识]，如果无法确定用户意图，则返回DEFAULT，同时需要将用户的问题一并返回，可以参考的格式为: json{    "destination": "公司知识","next_inputs": "我想咨询公司知识"}。用户的问题是:```云桌面黑屏怎么报修'''}]
    spark = ChatSparkLLM(
        spark_api_url='ws://10.103.240.12:15597/v1.1/chat',
        spark_app_id='00e5235f',
        spark_api_key='3d9766af2e83ef33fb2447e76bfb9d84',
        spark_api_secret='ODU4YjczOTI2NjZmMmJkYjYyMWI3MzJm',
        spark_llm_domain='xgemma29bit',
        streaming=False,
        model_kwargs={"patch_id": "1833085573603131392"},

    )
    fc = [
        {
            "name": "meet_assistant",
            "description": "会议助手， 负责调用会议助手",
            "parameters": {
                "type": "object",
                "properties": {
                    "next_inputs": {
    ...
    a = spark.generate([messages], callbacks=[], function_definition=fc)
    print(a)
