...
from sparkai.core._api.internal import is_caller_internal


class SparkAIDeprecationWarning(DeprecationWarning):
    """A class for issuing deprecation warnings for LangChain users."""


class SparkAIPendingDeprecationWarning(PendingDeprecationWarning):
    """A class for issuing deprecation warnings for LangChain users."""
...
def suppress_sparkai_deprecation_warning() -> Generator[None, None, None]:
    """Context manager to suppress LangChainDeprecationWarning."""
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", SparkAIDeprecationWarning)
        warnings.simplefilter("ignore", SparkAIPendingDeprecationWarning)
        yield
...
def warn_deprecated(
    ...
    *,
    ...
    if not message:
        message = ""
        package = name.split(".")[0].replace("_", "-") if "." in name else "sparkai"
    ...
    warning_cls = (
        SparkAIPendingDeprecationWarning if pending else SparkAIDeprecationWarning
    )
    warning = warning_cls(message)
    warnings.warn(warning, category=SparkAIDeprecationWarning, stacklevel=2)


def surface_sparkai_deprecation_warnings() -> None:
    """Unmute LangChain deprecation warnings."""
    warnings.filterwarnings(
        "default",
        category=SparkAIPendingDeprecationWarning,
    )

    warnings.filterwarnings(
        "default",
        category=SparkAIDeprecationWarning,
    )
